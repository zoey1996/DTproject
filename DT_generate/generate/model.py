...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        num = int(bool(config.pro_len))  #int(config.lstm_layers)    #  int(config.scaffold) 
        # num = 1
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.pro_len:
            self.pro_nn = nn.Linear(config.pro_len, config.n_embd)
    ...
    def forward(self, idx, targets=None, pro = None):
        b, t = idx.size()
        assert t <= self.block_size, "Cannot forward, model block size is exhausted."

        if self.config.pro_len:
            assert pro.size(-1) == self.config.pro_len, "Num_props should be equal to last dim of property vector"           
        ...
        if self.config.pro_len:
            ...
            if pro.ndim == 2:
                p = self.pro_nn(pro.unsqueeze(1))    # for single property
            else:
                p = self.pro_nn(pro)    # for multiproperty
            p += type_embd
            x = torch.cat([p, x], 1)
        ...
        if self.config.pro_len:
            num = int(bool(self.config.pro_len))
        else:
            num = 0
